...
void SM4_T_table(uint8_t* input, uint8_t* output, uint32_t* rk, bool is_enc) {
	uint32_t A[4];
	uint32_t temp = 0;
	...
	for (int i = 0; i < 32; i++) {
		SM4_T_table_1R(i, A, rk, is_enc, temp);
	}
	uint32_t tmp = A[0];
	A[0] = A[3];
	A[3] = tmp;
...
int main() {
	...
	printf("=====================SM4_T_table加解密=====================\n");
	printf("密文：\n");
	SM4_T_table(input, input, rk, 1);
	for (int i = 0; i < 16; i++) {
		printf("%02x ", input[i]);
	}
	printf("\n");
	...
	printf("明文：\n");
	SM4_T_table(input, input, rk, 0);
	for (int i = 0; i < 16; i++) {
		printf("%02x ", input[i]);
	}
	printf("\n");

	printf("=====================SM4_T_table性能测试=====================\n");
